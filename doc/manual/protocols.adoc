...
SYM_ENCRYPT uses store type JCEKS by default.
To generate a keystore in JCEKS format with keytool, use the following command:
...
If you create a keystore in a different format, specify it with the `keystore_type` attribute.
...
NOTE: Both SYM_ENCRYPT and ASYM_ENCRYPT should be placed directly above NAKACK2
